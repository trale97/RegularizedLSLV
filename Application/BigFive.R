...
V_a <- sum((CCLSLV_model$scores %*% t(CCLSLV_model$loadings))^2)
V_oo <- sum(DATA^2)
PEV <- V_a/V_oo #formula from Zhengguo
# double check with another formula
PEV <- 1-(CCLSLV_model$Loss/V_oo)  
...
# unshrink nonzero loadings
reestimated <- LSLVL_undoshrinkage(DATA, R=5, P=LSLVLASSO_loadings, MaxIter=50,eps=1e-4)
V_a <- sum((reestimated$loadings %*% t(reestimated$scores))^2)
V_oo <- sum(DATA^2)
PEV <- V_a/V_oo #formula from Zhengguo 
PEV <- 1-(reestimated$Loss/V_oo)  

cor(reestimated$scores)
