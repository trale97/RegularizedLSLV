...
model5_list <- list(model5_regsem1, model5_regsem2, model5_regsem3, model5_regsem4, model5_regsem5, model5_regsem6)
...
Simulation_regSEM7 = foreach(i = 701:900,
                             ...
                             .combine = rbind)%dopar%{
                               ...
                               corrate1 <- vector(length = length(model5_list))
                               corrate2 <- vector(length = length(model5_list))
                               spcr_tucongrT1 <- vector(length = length(model5_list))
                               spcr_tucongrT2 <- vector(length = length(model5_list))
                               
                               
                               for (j in 1:length(model5_list)){
                                 fit1 <- sem(model5_list[[j]], data = data1, se = "none")
                                 ...
                                 beta1 <- unname(reg.out1$final_pars[46:48])
                                 beta1_list[[j]] <- beta1
                                 
                                 fit2 <- sem(model5_list[[j]], data = data2, se = "none")
                                 ...
                                 beta2 <- unname(reg.out2$final_pars[46:48])
                                 beta2_list[[j]] <- beta2
