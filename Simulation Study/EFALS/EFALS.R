## EFALS 
# model ||X - TP^T - UPHI||^2  = ||X - BH^T|| subject to T^TT = N, U^TU = N, F^TU = O

##1. EFALS function
EFALS <- function(DATA, R, CARD, MaxIter, eps){
  J <- dim(DATA)[2]
  I <- dim(DATA)[1]
  convAO <- 0
  iter <- 1
  ...
  scores <- matrix(runif(I*R, 0 , 1) - 0.5, ncol = R, nrow = I)
  U <- matrix(runif(I*J, 0 , 1) - 0.5, ncol = J, nrow = I)
  loadings <- INITLOADINGS(DATA, R, CARD)
  PHI <- matrix(rep(0,J*J),J,J)
  diag(PHI) <- diag(t(U)%*%DATA)
  H <- cbind(loadings, PHI)
  A <- loadings
  if (sum(length(CARD)) > 1){
    ...
    while (convAO == 0) {
      ...
      H <- cbind(loadings,PHI)
      XH <- DATA%*%H/sqrt(I)
      B <- sqrt(I) * svd(XH, (J+R), (J+R))$u %*% t(svd(XH, (J+R), (J+R))$v)
      scores <- B[,c(1:R)]
      U <- B[,(R+1):(R+J)]
      A <- t(DATA) %*% scores/ I
      diag(PHI) <- diag(t(U)%*%DATA)/I
      H <- cbind(A, PHI)
      #Calculate loss
      Loss <- RESIDUAL_EFALS(DATA,scores,loadings,U,PHI)
      ...
      #Calculate loss
      Lossu <- RESIDUAL_EFALS(DATA,scores,loadings,U,PHI)
  ...
  efals <- list('scores' = scores, 'loadings' = loadings,'U' = U, 'PHI' = PHI, 'Lossvec' = Lossvec, 'Residual' = Loss)
  return(efals)
...
##2. LOSS
RESIDUAL_EFALS <- function(DATA, SCORES, LOADINGS, U, PHI){
  XHAT <- SCORES%*%t(LOADINGS) + U%*%PHI
...
#########################################################################################
MULTISTART_EFALS <- function(DATA, R, CARD, MaxIter, eps, nstarts){
  if(missing(nstarts)){
    nstarts <- 20
  } 
  ...
  for (n in 1:nstarts){
    result <- EFALS(DATA, R, CARD, MaxIter, eps)
    
    Pout3d[[n]] <- result$loadings
    Tout3d[[n]] <- result$scores
    Uout3d[[n]] <- result$U
    PHIout3d[[n]] <- result$PHI
  ...
  return_varselect$U <- Uout3d[[k]]
  return_varselect$PHI <- PHIout3d[[k]]
