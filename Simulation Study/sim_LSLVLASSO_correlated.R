...
Simulation3 = foreach(i = 1:Ndatasets,
                      ...
                      .combine = rbind)%dopar%{
                        ...
                        for (j in 1:length(lambdavec1)){
                          IS1[j] <- IS_RLSLV(data1, R = 3, lambda = lambdavec1[j], MaxIter = 200, eps = 10^-6, nstarts = 10)
                        }
                        for (k in 1:length(lambdavec2)){
                          IS2[k] <- IS_RLSLV(data2, R = 3, lambda = lambdavec2[k], MaxIter = 200, eps = 10^-6, nstarts = 10)
                        }
                        
                        result1 <- MULTISTART(DATA = data1, R = 3, lambda = lambdavec1[which.max(IS1)], MaxIter = 200, eps = 10^-6, nstarts = 20)
                        result2 <- MULTISTART(DATA = data2, R = 3, lambda = lambdavec2[which.max(IS2)], MaxIter = 200, eps = 10^-6, nstarts = 20)
                        ...
                        save(output, file = paste0("CORRELATED2/RLSLV", i, ".RData"))
